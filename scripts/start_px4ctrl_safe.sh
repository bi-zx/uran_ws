...
python3 "${WORKSPACE_DIR}/scripts/wait_for_px4ctrl_rc_safe.py" \
  --topic "${PX4CTRL_RC_TOPIC:-/mavros/rc/in}" \
  --stable-seconds "${PX4CTRL_RC_STABLE_SECONDS:-2.0}" \
  --timeout "${PX4CTRL_RC_CHECK_TIMEOUT:-0}" \
  --max-age "${PX4CTRL_RC_MAX_AGE:-1.0}" \
  --throttle-min "${PX4CTRL_RC_THROTTLE_MIN:-1400}" \
  --throttle-max "${PX4CTRL_RC_THROTTLE_MAX:-1600}" \
  --ch5-min "${PX4CTRL_RC_CH5_MIN:-1900}" \
  --ch5-max "${PX4CTRL_RC_CH5_MAX:-2100}" \
  --ch6-min "${PX4CTRL_RC_CH6_MIN:-1900}" \
  --ch6-max "${PX4CTRL_RC_CH6_MAX:-2100}"

echo "RC startup gate passed; launching px4ctrl"
